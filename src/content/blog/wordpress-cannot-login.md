...
<p><strong>URLバーだけで切り分けられます</strong></p>
<p><code>/wp-admin/</code> に変わっていれば認証は成功している（＝プラグインやPHPのエラー）。<code>/wp-login.php</code> のままなら認証が通っていないかCookieが保持できていない（＝ログインループ）。</p>
</aside>
...
<p><strong>この時点でやってはいけないこと</strong></p>
<p>WordPressの再インストール、ファイルの削除、プラグインフォルダの削除。ログインできないだけの状態を、本当にデータを失う状態に変えてしまうのは、たいていこの操作です。</p>
</aside>
...
<p><strong>触る前に必ずバックアップを</strong></p>
<p>phpMyAdminの「エクスポート」タブからデータベース全体をSQL形式で書き出し、パソコンに保存してから作業してください。これが無い状態で触るのは、命綱なしで屋根に上るのと同じです。</p>
</aside>
...
<aside class="callout callout-danger">
<p><strong>phpMyAdminで触ってはいけないもの</strong></p>
<p>「SQL」タブと、行の削除ボタン。初心者が事故を起こすのは、ほぼこの2つです。編集は必ず、鉛筆マークの「編集」ボタンから行ってください。</p>
</aside>
...
<aside class="callout callout-danger">
<p><strong>ユーザー一覧が正常でも、安心はできません</strong></p>
<p>身に覚えのないパスワード変更の通知メールが届いている、サイトの表示に文字化けや知らない広告が出ている、パスワードを変えてもまた入れなくなる。このいずれかに当てはまるなら、アカウントが増えていなくても乗っ取りを疑ってください。</p>
</aside>
...
<aside class="callout callout-warn">
<p><strong>wp-config.php を編集するときは</strong></p>
<p>データベースの接続情報を含む最重要ファイルです。編集前に必ず原本をパソコンにダウンロードして保管し、メモ帳ではなくテキストエディタ（UTF-8・BOMなし）で編集してください。<strong>全角スペースが1つ混入するだけで、サイト全体が停止します。</strong></p>
</aside>
...
<aside class="callout callout-warn">
<p><strong>WAFをオフにするのは「検査」です</strong></p>
<p>原因の特定が終わったら、必ずオンに戻してください。オフのままにするのは、サイトの前に立っていた警備員を帰らせた状態です。恒久対応は、全体をオフにすることではなく、誤検知したルールだけを除外することです。</p>
</aside>
...
<p><strong>この2行を書くと、管理画面のURL欄が編集できなくなります</strong></p>
<p>仕様通りの動作ですが、これを知らないと後日「なぜかURLが変更できない」と延々悩むことになります。データベース側を直したら、この2行は削除してください。</p>
</aside>
